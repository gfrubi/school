	...
	contains
        ...
        subroutine dersecond2(u,du,dx, Ntot, Nx)
        ...
        du(1) = (2.*u(1)-5.*u(2)+4.*u(3)-u(4))/dx**2
        du(nx) = (2.*u(nx)-5.*u(nx-1)+4.*u(nx-2)-u(nx-3))/dx**2

        end subroutine dersecond2

!this is just a wrapper, it calls the ders later	
!Ntot is total number in array, Nx up to IMAX
	subroutine derivs(u,du,dx, Ntot,dir,Nx)
	use m_pars, only : derorder
	real*8, dimension(:), intent(in) :: u
	real*8, dimension(:), intent(inout) :: du
	real*8 :: dx
	integer :: i, j, nx,dir, Ntot
	   
	    du = 0.0

		if (derorder.eq.1) then
		  call derivs12(u(:),du(:),dx,Ntot,NX)
		else if(derorder.eq.2) then
		  call derivs24(u(:),du(:),dx,Ntot,NX)
		else if(derorder.eq.3) then
		  call derivs36(u(:),du(:),dx,Ntot,NX)
		else if(derorder.eq.4) then
		  call derivs48(u(:),du(:),dx,Ntot,NX)
		else if(derorder.eq.41) then
		  call derivs48_b(u(:),du(:),dx,Ntot,NX)
		end if
             if(Nx.lt.Ntot) du(Nx+1:Ntot) = du(Nx)

	end subroutine derivs
	
	subroutine derivs12(u,du,dx, Ntot, Nx)
	real*8, dimension(:), intent(in) :: u
	real*8, dimension(:), intent(inout) :: du
	real*8 :: dx
	integer :: i, Ntot, nx
...
!	du(nx) = 0.5*(u(2)-u(nx-1))/dx
	...
	subroutine dissip(u,du,dx, Ntot,aa,Nx)
	use m_pars, only : dissorder
	
	real*8, dimension(:), intent(in) :: u
	real*8, dimension(:), intent(inout) :: du
	real*8 :: dx
	integer :: i, j, Nx,aa,Ntot
...
!do each dimension and then add
		if (dissorder.eq.4) then
		    call dissip4(u(:),du(:),dx,Ntot,NX)
		else if(dissorder.eq.6) then
		    call dissip6(u(:),du(:),dx,Ntot,NX)
		else if(dissorder.eq.8) then
		   call dissip8(u(:),du(:),dx,Ntot,NX)	
		end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	subroutine dissip4(u,du,dx, Ntot,Nx)
	use m_pars, only : derorder		
	real*8, dimension(:), intent(in) :: u
	real*8, dimension(:), intent(inout) :: du
	real*8 :: dx
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	subroutine dissip6(u,du,dx, Ntot,Nx)	
	use m_pars, only : derorder			
	real*8, dimension(:), intent(in) :: u
	real*8, dimension(:), intent(inout) :: du
	real*8 :: dx
	integer :: i, Nx, lef, rig,Ntot			
	real*8 :: smr3,smr2,smr1,spr1,spr2,spr3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	subroutine dissip8(u,du,dx, Ntot,Nx)	
	use m_pars, only : derorder			
	real*8, dimension(:), intent(in) :: u
	real*8, dimension(:), intent(inout) :: du
	real*8 :: dx
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!
	subroutine derivs24(u,du,dx,Ntot,Nx)
	real*8, dimension(:), intent(in) :: u
	real*8, dimension(:), intent(inout) :: du
	real*8 :: dx
	integer :: i, nx,Ntot
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	subroutine derivs36(u,du,dx,Ntot,Nx)
	real*8, dimension(:), intent(in) :: u
	real*8, dimension(:), intent(inout) :: du
	real*8 :: dx
	integer :: i, nx,Ntot
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!		
	subroutine derivs48(u,du,dx,Ntot,Nx)
	real*8, dimension(:), intent(in) :: u
	real*8, dimension(:), intent(inout) :: du
	real*8 :: dx
	integer :: i, nx,Ntot		
...
!!!!!!!!!!!!!!!! 4th in the bound, 8th in the interior
!!!!!!!!!!!!!!!!!!!!!!!!!!!		
	subroutine derivs48_b(u,du,dx,Ntot,Nx)
	real*8, dimension(:), intent(in) :: u
	real*8, dimension(:), intent(inout) :: du
	real*8 :: dx
	integer :: i, nx,Ntot		
